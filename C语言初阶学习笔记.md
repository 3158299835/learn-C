...
2. `断点可以右键`暂时关闭，也可以右键断点设置触发断点的条件
3. 数组传参后输入`数组名，数字`。就可以看到数组的n个元素了。而不止是首元素
...
1. 使用**assert ，//断言。判断然后警告**
2. 尽量使用const  //这个是用来放置被修改，来保护他的内容
const 修饰指针时的一些理解
**const 修饰int* p 则*p所指向的对象不能修改，但是p所存的地址可以修改
int* const 修饰 P 则 p所存的地址不能修改，但可以修改*p所指向的对象**
3. 养成良好的编码风格
4. 添加必要的注释
5. 避免编码的陷阱。
